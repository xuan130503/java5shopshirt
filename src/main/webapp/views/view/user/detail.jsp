			...
			<div class="col-md-4 cart">
				<div class="">
					<div class="tshirt_img">
						<img width="350px"
							src="<c:url value='/views/templates/user/images/${product.image}'/>">
					</div>
				</div>
			</div>
					...
					<div class="buy_bt">
						<form action="/user/store" method="POST">
							<input name="product" type="hidden" value="${ product.id }" /> <input
								...
								<h1>${ product.name }</h1>
								<h4>Info: ${product.descriptions}</h4>
								<h4 class="text-danger fw-bold">Price: ${ product.price } &dollar;</h4>
								Quantity:
								<input type="number" style="width:70px" class="form-control" name="quantity" value="${ quantity }" />
								<h5>
									Status: <span class="text-secondary">${ product.available?'Available':'Sold out' }</span>
								</h5>
